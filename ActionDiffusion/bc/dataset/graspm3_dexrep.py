        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="Multi-threaded loading"):
                ...
                if sum(len(d['grasp_seqs']) for d in all_results if 'grasp_seqs' in d) >= target_seq_num:
                    break
        if os.environ.get("DEXGRASP_DETERMINISTIC_LOAD_ORDER", "1") in {"1", "true", "True", "yes", "on"}:
            order = {name[:-4]: idx for idx, name in enumerate(obj_id_list)}
            all_results.sort(key=lambda item: order.get(item.get('obj_code', ''), len(order)))
